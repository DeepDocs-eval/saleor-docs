...
```json
{
  ...
  "refundReasonReference": "UGFnZTox"  # ID of a Page representing the reason for the refund
```
</TabItem>
</Tabs>
...
You can decide you want a strict policy to require a reason reference for every refund. Such reference will be a relation to a `Page` type. To enable that, you need first to create a `PageType` dedicated for such refunds. You can do that in the Dashboard or by `pageTypeCreate` mutation.
...
Apart from that, the `reasonReference` provided in refund mutations must be a valid `Page` ID of the type set in the `refundReasonReferenceType` setting. If not, a ValidationError will be raised as well.
